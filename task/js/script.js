...
const main = create(
  'main',
  'wrapper',
  [create('h1', 'title', 'Віртуальная клавіятура'), create('h3', 'subtitle', 'Клавіятура распрацавана ў аперацыйнай сістэме Windows'), create('p', 'description', 'Для змены раскладкі выкарыстоўвай Ctrl + Shift'),
  ],
);
...
document.onkeydown = function shift(evt) {
  if (evt.code === 'ShiftLeft' || evt.code === 'ShiftRight') {
    document.onkeyup = function control(event) {
      if ((event.code === 'ControlLeft' || event.code === 'ControlRight') && lang === 'by') {
        ...
        allKeys.forEach((el) => {
          ...
          for (let i = 0; i < language[lang].length; i += 1) {
